...
sidebarTitle: "Styling and Layout"
description: ""
...
Docusaurus uses `996px` as the <span style={{color: "var(--secondary-font-color)"}}> cutoff </span> between mobile screen width and desktop.
...
Webpack will load such CSS files as CSS modules and you have to reference the class names as <span style={{color: "var(--secondary-font-color)"}}> properties </span> of the <span style={{color: "var(--secondary-font-color)"}}> imported CSS module </span>.

``` css title="styles.module.css" lines
.main {
  padding: 12px;
}
...
The class names will be processed by webpack into a <span style={{color: "var(--secondary-font-color)"}}> globally unique class name </span> during build.
