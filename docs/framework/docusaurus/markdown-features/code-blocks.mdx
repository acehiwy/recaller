...
You can check our implementations for the [light](https://github.com/facebook/docusaurus/blob/main/website/src/utils/prismLight.ts) and [dark](https://github.com/facebook/docusaurus/blob/main/website/src/utils/prismDark.ts) code block themes.
...
For example, Prism would map the language `cs` to `csharp`, but only `prism-csharp.js` <span style={{color: "var(--primary-font-color)"}}> exists </span> as a component
...
``` `markdown lines highlight={4,11-13}
```js
...
``` css title="/src/css/custom.css" lines
:root {
...
`// highlight-next-line` and `// highlight-start` etc. are called <span style={{color: "var(--secondary-font-color)"}}> "magic comments" </span>.
...
<Tabs>
<Tab title="docusaurus.config.js">

``` javascript lines highlight={11}
module.exports = {
  themeConfig: {
    prism: {
      magicComments: [
...
You can enable line numbering for your code block by using `showLineNumbers` key within the language meta string <span style={{color: "var(--secondary-font-color)"}}> (don't forget to add space directly before the key) </span>.
