...
sidebarTitle: "Reusing Descriptions"
tags:
  - openapi
...
It contains <span style={{color: "var(--secondary-font-color)"}}> definitions for objects to be reused </span> in other parts of the document.
...
Each field in the Components Object is a map pairing <span style={{color: "var(--secondary-font-color)"}}> component names </span>(as a key) with objects(as a value) to be reused.

The <span style={{color: "var(--secondary-font-color)"}}> type of these objects must match the parent field </span>, e.g. objects in the `schemas` map must be Schema Objects.
...
Any OpenAPI object of the types <span style={{color: "var(--secondary-font-color)"}}> supported by </span> the Components Object can be replaced by a [Reference Object](https://spec.openapis.org/oas/v3.1.0#reference-object) pointing to a component.

Reference Objects are actually [JSON References](https://tools.ietf.org/html/draft-pbryan-zyp-json-ref-03)

They contain <span style={{color: "var(--secondary-font-color)"}}> a single field </span> named `$ref` and its string value is a <span style={{color: "var(--secondary-font-color)"}}> URI pointing to the referenced object </span>:
