...
sidebarTitle: "Content of Message Bodies"
tags:
  - openapi
...
This allows returning / accepting content in <span style={{color: "var(--secondary-font-color)"}}> different formats </span>, each one with a different structure described by the [Media Type Object](https://spec.openapis.org/oas/v3.1.0#media-type-object).
...
The [Media Type Object](https://spec.openapis.org/oas/v3.1.0#media-type-object) describes the <span style={{color: "var(--secondary-font-color)"}}> structure of the content </span> and <span style={{color: "var(--secondary-font-color)"}}> provides examples </span> for documentation and mocking purposes.
...
The [Schema Object](https://spec.openapis.org/oas/v3.1.0#schema-object) <span style={{color: "var(--secondary-font-color)"}}> defines a data type </span> which can be a primitive (integer, string, …), an array or an object depending on its `type` field.
...
**<span style={{color: "var(--writer-additional-note-font-color)"}}> //\\\\ </span>** All items inside array have to be the same type.
...
Object types <span style={{color: "var(--secondary-font-color)"}}> must have </span> a `properties` field listing the properties of the object.
