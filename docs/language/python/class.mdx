...
&#8203;<span style={{color: "var(--not-require-but-good-to-know-font-color)"}}>Code below is used as reference on futher section on this page</span>
...
When method is called, <span style={{color: "var(--secondary-font-color)"}}>an instance is passed as the first argument</span>
...
When attribute is referenced, Python does depth-first / left-to-right search and <span style={{color: "var(--secondary-font-color)"}}>not searching for same class twice</span>

Eg. if attribute `foo` is called, python will search from
1. `DerivedClassName`
1. `Base1` class
    1. Parent class of `Base1` and continue <span style={{color: "var(--secondary-font-color)"}}>recursivly</span> through its grandparent class using same left-to-right searching
1. `Base2` class
1. `Base3` class

<Accordion title="not searching for same class twice example">
...
## <span style={{color: "var(--not-require-but-good-to-know-font-color)"}}>Attribute references</span>
