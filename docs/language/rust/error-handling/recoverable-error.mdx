...
sidebarTitle: "Recoverable Errors"
...
- The `Ok` variant indicates the operation was successful, and <span style={{color: "var(--secondary-font-color)"}}> inside </span> `Ok` is the successfully generated value (`T`).
...
`Result` has `unwrap` and `expect` methods.
...
The `?` operator can only be used <span style={{color: "var(--secondary-font-color)"}}> in a function </span> that returns `Result` or `Option` (or another type that implements `FromResidual`)
...
### <span style={{color: "var(--not-nesary-for-now-future-font-color)"}}> Main function return type </span>
