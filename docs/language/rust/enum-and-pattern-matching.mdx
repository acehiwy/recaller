---
title: "Enums and Pattern Matching"
sidebarTitle: "Enums and Pattern Matching"
tags:
  - rust
lastUpdated: 2026-04-08T09:00:42.959Z
---

import { Callout } from '/snippets/callout.mdx';

Enums allow you to <span style={{color: "var(--secondary-font-color)"}}> define a type </span> by enumerating its possible variants.

``` rust lines
enum IpAddrKind {
    V4,
    V6,
}
```

## Enum value

``` rust lines
enum IpAddrKind {
    V4, // 0
    V6, // 1
}

fn main(){
  let four = IpAddrKind::V4;
  let six = IpAddrKind::V6;

  route(four);
  route(IpAddrKind::V6);
}

// accept enum type
fn route(ip_kind: IpAddrKind) {
  // do something
}
```

We attach data to each variant of the enum directly

``` rust lines
enum Message {
    Quit, // value will be integer (in this case 0)
    Move { x: i32, y: i32 }, // struct as a value
    Write(String), // tuple as a value
    ChangeColor(i32, i32, i32), // tuple as a value
}

fn main(){
  let msg_quit = Message::Quit;
  let msg_move = Message::Move{
    x: 12,
    y: 65
  };
  let msg_change_color = Message::ChangeColor(10,20,30);
}

enum Enum {
    Foo, // 0
    Bar, // 1
    Baz, // 2
}

assert_eq!(0, Enum::Foo as isize);
assert_eq!(1, Enum::Bar as isize);
assert_eq!(2, Enum::Baz as isize);

enum Foo {
    Bar,            // 0
    Baz = 123,      // 123
    Quux,           // 124
}

let baz_discriminant = Foo::Baz as u32;
assert_eq!(baz_discriminant, 123);
```

<Callout type="info">

You can put any kind of data inside an enum variant:

- strings
- numeric
- structs
- enum

</Callout>

<span style={{color: "var(--not-require-but-good-to-know-font-color)"}}>

Defining `Enum` with different type of its variants is similar to defining different kind of struct. Different is that all variants are grouped under the Enum type

</span>

``` rust lines
struct QuitMessage; // unit struct
struct MoveMessage {
    x: i32,
    y: i32,
}
struct WriteMessage(String); // tuple struct
struct ChangeColorMessage(i32, i32, i32); // tuple struct
```

<span style={{color: "var(--not-require-but-good-to-know-font-color)"}}>

Also if we used the different structs, each of which has its own type, we
couldn’t as easily define a function to take any of these kinds of messages as
we could with the Message enum, which is a single type.

</span>

``` rust lines
fn foo(bar: Message){
    // do something
}
```

## Defining Method

We can also define method on enums using `impl`

``` rust lines
fn main() {
    enum Message {
        Quit,
        Move { x: i32, y: i32 },
        Write(String),
        ChangeColor(i32, i32, i32),
    }

    impl Message {
        fn call(&self) {
            // method body would be defined here
        }
    }

    let m = Message::Write(String::from("hello"));
    m.call();
}
```

## `Option` Enum

Variable that have type `Option<T>` means it either contain `T` or `None` (a value that can possibly be null)

In order to use a value `T`, you are required to explicitly handle the case when the value is null.

``` rust lines
enum Option<T> {
    None,
    Some(T),
}
```

``` rust lines
let some_number = Some(5); // type of some_number is Option<i32>
let some_char = Some('e'); // type of some_char is Option<char>

let absent_number: Option<i32> = None;
```

<Callout type="info">

`Option` enum is an enum defined by the standard library.

`Some` and `None` variants are also include in the prelude.

You can use directly without the `Option::` prefix.

</Callout>

## `match` Expression

`match` allows you to compare a value against a series of patterns and then execute code based on which pattern matches.

``` rust lines
enum Coin {
    Penny,
    Nickel,
    Dime,
    Quarter,
}

fn value_in_cents(coin: Coin) -> u8 {
    match coin {
        Coin::Penny => {
            println!("Lucky penny!");
            1
        }
        Coin::Nickel => 5,
        Coin::Dime => 10,
        Coin::Quarter => 25,
    }
}


```

The code associated with each arm is <span style={{color: "var(--secondary-font-color)"}}> an expression </span>,
and the resultant value of the expression in the matching arm is the value that gets <span style={{color: "var(--secondary-font-color)"}}> returned for </span> the entire <span style={{color: "var(--secondary-font-color)"}}> match expression </span>.

`match` end when a particular pattern is matched.

<Callout type="info">

Every possible case of `match` need to be handled.

</Callout>

### Tuple variant enum with `match`

If your enum variant have a tuple as a value, you can use this syntax to access the variant's value:

``` rust lines
#[derive(Debug)]
enum UsState {
    Alabama,
    Alaska,
    // --snip--
}

enum Coin {
    Penny,
    Nickel(String),
    Dime { key1: u32, key2: u32 },
    Quarter(UsState, UsState),
}

fn main() {
    let foo = value_in_cents(Coin::Quarter(UsState::Alaska, UsState::Alabama));
    let foo = value_in_cents(Coin::Nickel("whatever".to_string()));
    let foo = value_in_cents(Coin::Dime { key1: 10, key2: 20 });
}

fn value_in_cents(coin: Coin) -> u8 {
    match coin {
        Coin::Penny => 1,
        Coin::Nickel(some_string) => { // match tuple String
            println!("{some_string}");
            5
        }
        Coin::Dime { key1, key2 } => 5, // match struct
        Coin::Quarter(state, state2) => { // match tuple enum
            // variable `state` represent UsState value contain inside Coin::Quarter
            println!("State quarter from {:?}!", state);
            25
        }
    }
}

```

`state` value will be `UsState::Alaska`

### Matching with `Option<T>`

``` rust lines
fn plus_one(x: Option<i32>) -> Option<i32> {
    match x {
        None => None,
        Some(i) => Some(i + 1),
    }
}

let five = Some(5);
let six = plus_one(five);
let none = plus_one(None);
```

### Catch-all Pattern

Defining a variable as a pattern in `match` will match all values not specifically listed.

``` rust lines highlight={5-6}
let dice_roll = 9;
match dice_roll {
    3 => add_fancy_hat(),
    7 => remove_fancy_hat(),
    // other value that are not handled will be binded to `some_variable_name`
    some_variable_name => move_player(some_variable_name),
}

fn add_fancy_hat() {}
fn remove_fancy_hat() {}
fn move_player(num_spaces: u8) {}
```

If the value will not be used, prepend variable name with `_` or use `_` as a variable name

``` rust lines highlight={4}
match dice_roll {
    3 => add_fancy_hat(),
    7 => remove_fancy_hat(),
    _ => reroll(),
}
```

``` rust lines highlight={4}
match dice_roll {
    3 => add_fancy_hat(),
    7 => remove_fancy_hat(),
    _will_not_be_used => reroll(),
}
```

### `if let`

`if let` is a syntax sugar of `match` but it can express only 1 pattern per `if let`

``` rust lines
let config_max = Some(3u8);
match config_max {
    Some(max) => println!("The maximum is configured to be {}", max),
    _ => (),
}

// same as
let config_max = Some(3u8);
if let Some(max) = config_max {
    println!("The maximum is configured to be {}", max);
}
```

``` rust lines
let coin = Coin::Quarter(UsState::Alaska);
let mut bar = 0;


match coin {
    Coin::Quarter(state) => println!("State quarter from {:?}!", state),
    Coin::Dime => println!("Coin Dime"),
    _ => bar += 1,
}

// same as
if let Coin::Quarter(state) = coin {
    println!("State quarter from {:?}!", state);
}else if let Coin::Dime = coin {
    println!("Coin Dime");
}else {
    bar += 1;
};
```

<Callout type="note">

`if let` does not have the exhaustive checking that `match` enforces.

</Callout>

<br />

---

# Sources

- https://doc.rust-lang.org/book/ch06-01-defining-an-enum.html
- https://doc.rust-lang.org/book/ch06-02-match.html
- https://doc.rust-lang.org/book/ch06-03-if-let.html
