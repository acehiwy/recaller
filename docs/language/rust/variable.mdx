---
title: "Variables and Mutability"
sidebarTitle: "Variables and Mutability"
tags:
  - rust
lastUpdated: 2026-04-08T09:00:42.959Z
---

import { Callout } from '/snippets/callout.mdx';

## Variable

Variables are immutable by default

``` rust lines highlight={3}
let x = 5;

x = 10
// error[E0384]: cannot assign twice to immutable variable `x`
```

You can make them mutable by adding mut in front of the variable name

``` rust lines
let mut x = 5;
x = 6;
```

## Constants

``` rust
const THREE_HOURS_IN_SECONDS: u32 = 60 * 60 * 3;
```

Constants is like variable but they are always immutable. `mut` is not allowed to use with constant.

When defining constants, its type must be declared.

Constants can be declared in <span style={{color: "var(--secondary-font-color)"}}> any scope </span>, including the global scope.

Constants may be set only to a constant expression, <span style={{color: "var(--secondary-font-color)"}}> not the result of a value that could only be computed at runtime </span>.

<Callout type="info">

Rust’s naming convention for constants is to use all uppercase with underscores between words.

</Callout>

Constants are valid for the entire time a program runs, within the scope in which they were declared.

## Shadowing

You can declare a new variable <span style={{color: "var(--secondary-font-color)"}}> with the same name </span> as a previous variable.

Rustaceans say that the first variable is shadowed by the second. The second variable is valid until it itself is shadowed(by another variable) or the scope ends.

``` rust lines
fn main() {
    let x = 5;
    let x = x + 1;
    {
        let x = x * 2;
        println!("The value of x in the inner scope is: {x}");
        // The value of x in the inner scope is: 12
    }

    println!("The value of x is: {x}");
    // The value of x is: 6
}
```

Shadowing is different from marking a variable as `mut`.

- With shadowing we can perform a few transformations on a value but have the variable be immutable after those transformations have been completed (out of scope).
- With shadowing we are creating a new variable using `let`, the type of value can be change as well

``` rust lines
let spaces = "   ";
let spaces = spaces.len();
```

<br />

---

# Sources

- https://doc.rust-lang.org/book/ch03-01-variables-and-mutability.html
