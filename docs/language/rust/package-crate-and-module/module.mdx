...
Above module definetion will reflect as this module tree
...
- An absolute path is the full path starting <Color color="var(--secondary-font-color)"> from a crate root </Color>;
  ...
  - for code from the current crate, it starts with the literal `crate`.
    - eg. `create::<internal module>::<internal function>`
...
If the module itself is not marked public, the item it contained are still private even the item is marked public.

</Admonition>

<Admonition type="note" title="note">

Siblings modules / item can access each other without themselves marked public
...
The shortcut brought to the scope are private by default. Meaning outer ancestor / external code (lib user in this case) can not access the shortcut
