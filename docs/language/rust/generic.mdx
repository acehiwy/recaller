...
`<T: std::cmp::PartialOrd>` mean `T` can only be a type that implement `std::cmp::PartialOrd` <span style={{color: "var(--primary-font-color)"}}> trait </span>
...
## <span style={{color: "var(--not-nesary-for-now-future-font-color)"}}> Performance of Code Using Generics </span>
