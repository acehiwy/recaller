...
Instead we <span style={{color: "var(--secondary-font-color)"}}> schedule the task to be done later </span>.
...
We will use `setTimeout` to simulate a time-consuming task.

We'll use the number of dots in the string to represent one second of work. For example, a fake task described by `Hello...` will take three seconds.

### Producer

We will create `new_task.js` file. This program will schedule tasks to our work queue:

``` javascript title="new_task.js" lines highlight={12-21}
...
We will create a `worker.js` file. It will simulate one second of work for every dot in the message body.

``` javascript title="worker.js" lines highlight={14-32}
var amqp = require("amqplib/callback_api");
...
If we build up a backlog of work, we can simply add more workers and scale more easily.

First, let's try to run <span style={{color: "var(--secondary-font-color)"}}> two </span> `worker.js` scripts at the same time.
...
In the third shell, we'll publish new tasks.
...
``` text title="shell_1" lines
=> [x] Received 'First message.'
=> [x] Received 'Third message...'
=> [x] Received 'Fifth message.....'
```

``` text title="shell_2" lines
=> [x] Received 'Second message..'
=> [x] Received 'Fourth message....'
```

By default, RabbitMQ will <span style={{color: "var(--secondary-font-color)"}}> send each message </span> to the <span style={{color: "var(--primary-font-color)"}}> next </span> <span style={{color: "var(--secondary-font-color)"}}> consumer </span>,<span style={{color: "var(--primary-font-color)"}}> in sequence </span>.

On average every consumer will get the <span style={{color: "var(--primary-font-color)"}}> same number </span> of messages. This way of distributing messages is called round-robin.
...
We'll also <span style={{color: "var(--secondary-font-color)"}}> lose all the messages that were dispatched </span> to this particular worker <span style={{color: "var(--secondary-font-color)"}}> but were not yet handled </span>.

In order to make sure a message is never lost, RabbitMQ supports [message acknowledgments](../message-acknowledgment).

An ack(nowledgement) is <span style={{color: "var(--secondary-font-color)"}}> sent back </span> <span style={{color: "var(--primary-font-color)"}}> by the consumer </span> to tell RabbitMQ that a particular message has been received, processed and that RabbitMQ is <span style={{color: "var(--secondary-font-color)"}}> free to delete the message </span>.

If a consumer dies without sending an ack, RabbitMQ will understand that a message wasn't processed fully and will <span style={{color: "var(--secondary-font-color)"}}> re-queue </span> it.

A timeout (30 minutes by default) is enforced on <span style={{color: "var(--secondary-font-color)"}}> consumer </span> <span style={{color: "var(--primary-font-color)"}}> delivery </span> <span style={{color: "var(--secondary-font-color)"}}> acknowledgement </span>.
...
``` javascript title="worker.js" lines highlight={18-33}
var amqp = require("amqplib/callback_api");
...
Acknowledgement <span style={{color: "var(--secondary-font-color)"}}> must be sent </span> on the <span style={{color: "var(--primary-font-color)"}}> same channel </span> <span style={{color: "var(--secondary-font-color)"}}> that received the delivery </span>.
...
First, we need to make sure that the <span style={{color: "var(--secondary-font-color)"}}> queue </span> will <span style={{color: "var(--secondary-font-color)"}}> survive </span> a RabbitMQ <span style={{color: "var(--secondary-font-color)"}}> node restart </span>.
...
&#8203;<span style={{color: "var(--secondary-font-color)"}}>RabbitMQ doesn't allow you to redefine an</span> <span style={{color: "var(--primary-font-color)"}}>existing queue with different parameters</span>
...
This `durable` <span style={{color: "var(--secondary-font-color)"}}> option change needs to be applied </span> to <span style={{color: "var(--primary-font-color)"}}> both </span> <span style={{color: "var(--secondary-font-color)"}}> the producer and consumer </span> code.
...
Although <span style={{color: "var(--secondary-font-color)"}}> it tells RabbitMQ to save the message to disk </span>,
...
In order to defeat that we can use the `prefetch` method with the value of `1`. This tells RabbitMQ <span style={{color: "var(--secondary-font-color)"}}> not to give more than one message to a worker at a time </span>.
...
If all the workers are busy, <span style={{color: "var(--secondary-font-color)"}}> your queue can fill up </span>.
