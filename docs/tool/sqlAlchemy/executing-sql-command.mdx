...
If u pass list of param-dict to `execte()` method, the SQL stament will be invoked once for each item in the list
...
## Execute with ORM `Session`

in fact as the Session is used, it refers to a Connection internally which it uses to emit SQL.
